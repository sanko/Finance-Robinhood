...
has [
    'cumulative_quantity', 'pair_id',       'id', 'price', 'quantity', 'ref_id', 'side',
    'state',               'time_in_force', 'type'
];
...
SKIP: {
    ...
    todo( "Place an order and test if if can be cancelled then cancel it, reload, and retest it" =>
            sub { pass('ugh') } );
...
sub account ($s) {
    my $res = $s->_rh->_get( 'https://nummus.robinhood.com/accounts/' . $s->{account_id} . '/' );
    $res->is_success
        ? Finance::Robinhood::Forex::Account->new( _rh => $s->_rh, %{ $res->json } )
...
sub _test_account {
    ...
    isa_ok( t::Utility::stash('ORDER')->account, 'Finance::Robinhood::Forex::Account' );
}
...
sub _test_pair {
    t::Utility::stash('ORDER') // skip_all('No order object in stash');
    isa_ok( t::Utility::stash('ORDER')->pair, 'Finance::Robinhood::Forex::Pair' );
}
...
sub executions ($s) {
    map { Finance::Robinhood::Forex::Order::Execution->new( _rh => $s->_rh, %{$_} ) }
        @{ $s->{executions} };
}
...
sub reload($s) {
    my $res = $s->_rh->_get( 'https://nummus.robinhood.com/orders/' . $s->{id} . '/' );
    $_[0]
        = $res->is_success
