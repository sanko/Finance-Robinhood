...
has enrollment_state => (
    is      => 'ro',
    isa     => Enum [qw[upgrade_requested upgraded downgrade_requested downgraded unknown]],
    handles => [qw[is_upgrade_requested is_upgraded is_downgrade_requested is_downgraded]]
);
has is_active => (
    is      => 'ro',
    isa     => Bool,
...
has location_protection_enabled => ( is => 'ro', isa => Bool, coerce => 1, required => 1 );
