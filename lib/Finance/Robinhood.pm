...
sub _login ( $s, %opt ) {
    ...
    if ( $res->success ) {
        if ( $res->json->{mfa_required} ) {
            return $opt{mfa_callback} ? $res :
                $s->_login( %opt, mfa_code => $opt{mfa_callback}->( $res->json ) );
        }
        else {
...
sub search ( $s, $keyword ) {
    my $d
        = $s->_req( GET => 'https://midlands.robinhood.com/search/',
        query          => { query => $keyword } )->as(
...
sub _test_equity_fundamentals {
    my $rh = t::Utility::rh_instance(1);
    isa_ok( $rh->equity_fundamentals('MSFT')->current, 'Finance::Robinhood::Equity::Fundamentals' );
...
sub _test_tag {
    isa_ok( t::Utility::rh_instance(0)->collection('food'),
...
sub options_positions ( $s, %filters ) {
    ...
    $filters{chain_ids} = join ',',
        map { ref $_ eq 'Finance::Robinhood::Options::Chain' ? $_->id : $_ } @{ $filters{chains} }
...
sub _test_currency_halt_by_id {
    my $halt
        = t::Utility::rh_instance(1)->currency_halt_by_id('6a2a026a-e391-43cf-aadf-25826ea5432b');
...
sub _test_currency_portfolio_by_id {
    my $rh        = t::Utility::rh_instance(1);
    my $portfolio = $rh->currency_portfolio_by_id( $rh->currency_portfolios->current->id );  # Cheat
...
sub new_currency_application ( $s, %filters ) {
    ...
    my $res = $s->_req( POST => 'https://nummus.robinhood.com/activations/', form => \%filters );
}
...
Returns an iterator filled with C<Finance::Robinhood::Cash> obects.
...
Returns an iterator filled with C<Finance::Robinhood::Cash::Card> obects.
...
Returns a C<Finance::Robinhood::Cash::Card> obect.
